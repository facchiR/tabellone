...
a.tabellone = {};
a.tabellone.columns = []; //questo è un array di oggetti
a.tabellone.rows = []; //questo è un array di array
a.tabellone.binding = {};
...
function initBinding(data){
	...
	for (i=0;i<data.columns.length;i++){
		//creo la variabile data.columns[i].id dentro a binding e le assegno il relativo valore bind
		binding[data.columns[i].id] = data.columns[i].bind;
	}
	return binding;
}
...
//findRow(data, "nome", "Mario", "sesso","female")
function findRow (data,nome,stringa,field,fieldValue){
	var colonna = findColumn(data,nome);
	var tempField = findColumn(data,field);
	for(i=0;i<data.rows.length;i++){
		if(data.rows[i][colonna.bind]==stringa)
			return data.rows[i][tempField.bind]=fieldValue;
	}
}
